...
def test_get_entrance_fee():
    ...
    # Act 
    expected_entry_fee = Web3.toWei(0.025, "ether") # eth price = $2000 entry fee = $50 i.e in eth = 0.025
    fee = lottery.getEntranceFee()
    # Assert
    assert expected_entry_fee == fee
...
def test_can_choose_winner():
    ...
    # Here since we are on develpoment network so we have to pretend that we are a chainlink node
...
def main():
    ...
    test_can_enter_and_start_lottery()
    test_can_end_lottery()
    test_can_choose_winner()